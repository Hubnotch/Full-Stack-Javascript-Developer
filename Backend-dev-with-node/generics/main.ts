...
const menuQ = (): Promise<number> => {
    return new Promise((resolve, reject) => {
        ...
        rl.question('Your choice: ', (answer: number | PromiseLike<number>) => {
            resolve(answer);
        });
    });
};

const milkQ = (): Promise<number> => {
    return new Promise((resolve, reject) => {
        rl.question('How many cups of milk to add? ', (answer: number | PromiseLike<number>) => {
            resolve(answer);
        });
    });
};

const espressoQ = (): Promise<number> => {
    return new Promise((resolve, reject) => {
        rl.question('How many shots of espresso to add? ', (answer: number | PromiseLike<number>) => {
            resolve(answer);
        });
    });
};

const peppermintQ = (): Promise<number> => {
    return new Promise((resolve, reject) => {
        rl.question('How many shots of peppermint to add? ', (answer: number | PromiseLike<number>) => {
            resolve(answer);
        });
    });
};
...
const userOptions = async (
    ...
    const milkChoice:number = parseInt(milkPicked as unknown as string);
    const espPicked = await espressoQ();
    const espChoice = parseInt(espPicked as unknown as string);
    // If peppermint mocha
    if (mochaObject instanceof PeppermintMocha) {
        const pepPicked = await peppermintQ();
        const pepChoice = parseInt(pepPicked as unknown as string);
